...
class KakaoErrorDecoderTest extends Specification {
  ...
  def "에러디코더에서 에러발생 시 ApiException 예외가 발생한다."() {
    ...
    1 * objectMapper.readValue(*_) >> new KakaoErrorResponse("AccessDeniedError", "cannot find Authorization : KakaoAK header")
    ...
    verifyAll {
      e.errorMessage == "cannot find Authorization : KakaoAK header"
